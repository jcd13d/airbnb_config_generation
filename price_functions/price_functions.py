...
def get_day_config(config_template, start_day, end_day, s3_output_path):
...
def create_price_configs(config_template, skip_days, days_into_future, s3_output_path, out_path):
    pricing_configs = []
    days_to_config = get_config_days(days_into_future, skip_days)
    for start, end in zip(days_to_config[:-1], days_to_config[1:]):
        pricing_configs.append(get_day_config(config_template, start, end, s3_output_path))
